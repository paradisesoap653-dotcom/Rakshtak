...
export default function AdminDashboard() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#082f49] via-[#0c4a6e] to-[#0369a1] text-slate-100 p-3 pt-4 flex flex-col items-center justify-start">
      ...
      <div className="w-full max-w-md bg-[#0c4a6e] border border-sky-800/70 rounded-3xl p-4 shadow-2xl space-y-4">
        
        {/* مربعات الإحصائيات */}
        <div className="grid grid-cols-2 gap-2 text-center">
          <div className="bg-[#075985] border border-sky-700/60 p-2.5 rounded-2xl">
            <div className="text-[10px] text-slate-300">إجمالي الطلبات</div>
          ...
          <div className="bg-[#075985] border border-sky-700/60 p-2.5 rounded-2xl">
            <div className="text-[10px] text-sky-300">المكتملة</div>
            <div className="text-base font-bold text-sky-300 font-mono">{completedRides}</div>
          </div>
        </div>

        <div className="flex justify-between items-center pt-2 border-t border-sky-800/70">
          <h3 className="text-xs font-bold text-slate-300">سجل الطلبات الحية:</h3>
          <button
            type="button"
            onClick={fetchAllRides}
            className="text-[10px] bg-[#075985] hover:bg-[#0284c7] text-slate-300 px-2.5 py-1 rounded-lg transition"
          >
            تحديث 🔄
          </button>
        </div>
        ...
        <div className="space-y-3 max-h-[60vh] overflow-y-auto pr-1">
          {loading ? (
            <div className="text-center py-8 text-xs text-slate-400">جاري تحميل البيانات...</div>
          ) : rides.length === 0 ? (
            <div className="text-center py-8 text-xs text-slate-400">لا توجد طلبات مسجلة حتى الآن.</div>
          ) : (
            rides.map((ride) => (
              <div key={ride.id} className="bg-[#075985] border border-sky-800/70 rounded-2xl p-3 space-y-2 text-xs">
                <div className="flex justify-between items-center border-b border-sky-700/60 pb-2">
